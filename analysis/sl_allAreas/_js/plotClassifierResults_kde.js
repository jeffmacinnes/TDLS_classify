...
function drawSubjPlot(subj, subjData){
    ...
    // PMC ---------------
    var pmc_data = subjData['PMC']
    var pmc_data = subjData['PMC']
    var pmc_n = pmc_data.length
    var pmc_bins = d3.histogram().domain(xScale.domain())
            .thresholds(40)(pmc_data);
    var pmc_density = kernelDensityEstimator(kernelEpanechnikov(7), xScale.ticks(40))(pmc_data);
    var pmc_g = svg.append("g")
        .attr('class', 'pmc')
    pmc_g.append("path")
      .attr('class', 'PMC_path')
      .datum(pmc_density)
    ...
    // PPA ---------------
    var ppa_data = subjData['PPA']
    var ppa_data = subjData['PPA']
    var ppa_n = ppa_data.length
    var ppa_bins = d3.histogram().domain(xScale.domain())
            .thresholds(40)(ppa_data);
    var ppa_density = kernelDensityEstimator(kernelEpanechnikov(7), xScale.ticks(40))(ppa_data);
    var ppa_g = svg.append("g")
        .attr('class', 'ppa')
    ppa_g.append("path")
      .attr('class', 'PPA_path')
      .datum(ppa_density)
    ...
    var v2_path = svg.select('.V2_path')
      ...
      var v3_path = svg.select('.V3_path')
        ...
        var pmc_bins = d3.histogram().domain(xScale.domain())
                .thresholds(40)(pmc_data);
        var pmc_density = kernelDensityEstimator(kernelEpanechnikov(7), xScale.ticks(40))(pmc_data);
        var pmc_path = svg.select('.PMC_path')
          .datum(pmc_density)
          .transition()
          .duration(500)
          .attr("d",  d3.line()
        ...
        var ppa_bins = d3.histogram().domain(xScale.domain())
                .thresholds(40)(ppa_data);
        var ppa_density = kernelDensityEstimator(kernelEpanechnikov(7), xScale.ticks(40))(ppa_data);
        var ppa_path = svg.select('.PPA_path')
          .datum(ppa_density)
          .transition()
          .duration(500)
          .attr("d",  d3.line()
...
function updateClassifierLabel(){
    ...
    } else if (selectedClassProb == 'Stimulus'){
        d3.select("#classLabel").text('Unique stimulus')
    } else if (selectedClassProb == 'stimulusWords'){
        d3.select("#classLabel").text('Unique stimulus (words only)')
    } else if (selectedClassProb == 'stimulusPics'){
